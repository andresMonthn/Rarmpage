# Minimal AES-256 implementation for RAR Research
# Based on FIPS 197
# NOTE: This is slow and for educational/research purposes only.
...
def expand_key(master_key):
    ...
    key_columns = [0] * 60
    for i in range(8):
        key_columns[i] = (master_key[4*i] << 24) | (master_key[4*i+1] << 16) | \
                         (master_key[4*i+2] << 8) | master_key[4*i+3]
    
    for i in range(8, 60):
        temp = key_columns[i-1]
        if i % 8 == 0:
            temp = sub_word(rot_word(temp)) ^ r_con[i // 8]
        elif i % 8 == 4:
            temp = sub_word(temp)
        key_columns[i] = key_columns[i-8] ^ temp
    return key_columns
...
def aes_decrypt_block(ciphertext, key_schedule):
    ...
    add_round_key(state, key_schedule, 14)
...
class AES256Cipher:
    ...
    def decrypt_cbc(self, data, iv):
        ...
        for i in range(0, len(data), 16):
            block = data[i:i+16]
            if len(block) < 16: break # Padding handling is caller's job usually
            
            decrypted = aes_decrypt_block(block, self.key_schedule)
